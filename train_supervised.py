...
from similiarity_network3 import classifier_network5
...
def main(opt):
    ...
    # msg = model.load_state_dict(torch.load("fin_model15.pkl"))
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4, weight_decay=1e-5)  # Adam优化器
    scheduler = CosineLRScheduler(optimizer, t_initial=5, lr_min=1e-5,
    ...
    for epoch_flag in range(opt.epoch):
        ...
        torch.save(model.state_dict(), "fin_model1.pkl")
