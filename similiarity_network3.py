...
import numpy as np
# from src.xbert_change import BertConfig, BertModel, BertForMaskedLM
from xbert_change1 import BertConfig, BertModel, BertForMaskedLM


config = BertConfig.from_json_file('/home/10501002/BERT/CNTR/src/config_bert_supervised.json')
...
class classifier_network5(nn.Module):
    ...
    # Add a self-supervised task: MIM the Image to predict the MLM language
    def forward(self, image_input, text_inputs, idx, img_delta, txt_delta):
        ...
        image_input_cons = image_input[:,1:,:].repeat(len,1,1,1)
        ...
        cap_input_cons = cap_input[:,1:-1,:].repeat(len,1,1,1)
        ...
        cap_input_cons = cap_input_cons.permute(0,1,3,2)
        scores_bf = image_input_cons @ cap_input_cons   #len,len,36,cap_len
        scores_bf = F.softmax(scores_bf, dim=3)  # 8,8,36,12
        max_toal = torch.max(scores_bf, dim=3)
        max_num = max_toal.values  # 8,8,36
        max_index = max_toal.indices  # 8,8,36
        max_sum = torch.sum(max_num, dim=2)  # 8,8
        max_sum = F.softmax(max_sum, dim=1)

        loss_bf = Info_NCE(0.05, max_sum, pos_idx) + Info_NCE(0.05, max_sum.t(), pos_idx.t())
        ...
        cap_embeds_neg = []
        cap_neg_mask = []
        score_cap = torch.clamp(max_sum,0,1)
        for i in range(len):
            for j in range(len):
                if(int(pos_idx[i][j]) == 1):
                    score_cap[i][j] = -1
        max_cap_inx = torch.max(score_cap,dim=1)[1]
        for i in range(len):
            cap_embeds_neg.append(cap_input[ max_cap_inx[ i ] ])
            cap_neg_mask.append(key_padding_mask[ max_cap_inx[ i ] ])

        cap_neg_mask = torch.stack(cap_neg_mask, dim=0)
        cap_embeds_neg = torch.stack(cap_embeds_neg, dim=0)

        img_embeds_neg = [ ]
        text_embeds_neg = [ ]
        score_bf = torch.clamp(max_sum, 0, 1)
        for i in range(len):
            for j in range(len):
                if (int(pos_idx[ i ][ j ]) == 1):
                    score_bf[ i ][ j ] = -1
        ...
        text_embeds_neg_all = torch.cat([ cap_input, text_embeds_neg,cap_embeds_neg ], dim=0)
        # img_embeds_neg = torch.stack(img_embeds_neg, dim=0)  # 4, 37, 768
        img_embeds_neg = torch.stack(img_embeds_neg, dim=0)  # 4, 37, 768
        img_embeds_neg_all = torch.cat([ img_embeds_neg, image_input,image_input ], dim=0)

        text_attention_neg_all = torch.cat([ key_padding_mask, text_neg_mask,cap_neg_mask ], dim=0)
        img_attention_neg_all = torch.cat([ image_atts, image_atts,image_atts ], dim=0)
        ...
        img_txt_label = torch.cat([ torch.ones(3 * len, dtype=torch.long), torch.zeros(3 * len, dtype=torch.long) ],
                                  dim=0).cuda()

        loss_itm = F.cross_entropy(img_txt_score, img_txt_label)

        # mask region ->mask language
        input_ids = text_inputs.input_ids
        sample_index = np.random.choice(np.arange(0,36))
        image_mask = image_input
        image_mask[:,sample_index+1,:] = torch.zeros(image_mask.shape[2])
        select_index = torch.diagonal(max_index, dim1=0, dim2=1).t()  # len,36
        select_index = select_index[ :, sample_index ]  # len
        select_index = select_index + 1
        # print("select_index:", select_index)
        labels = text_inputs.input_ids
        for i in range(labels.shape[ 0 ]):
            labels[ i, select_index[ i ] ] = 103
        # print("labels", labels)
        # print(labels.shape)
        mlm_output = self.bert_model(input_ids,
                                     attention_mask=key_padding_mask,
                                     encoder_hidden_states=image_mask,
                                     encoder_attention_mask=image_atts,
                                     return_dict=True,
                                     labels=labels,
                                     alpha=0
                                     )
        loss_mim = mlm_output.loss
        ...
        # print("MLM input_ids:",input_ids)
        # print("MLM labels:",labels)
        mlm_output = self.bert_model(input_ids,
                                     attention_mask=key_padding_mask,
                                     encoder_hidden_states=image_input,
                                     encoder_attention_mask=image_atts,
        ...
        print("loss_mim:",loss_mim)
        loss_num = loss_itm + 0.1 * loss_bf + img_kl_loss + loss_mlm + loss_mim
    ...
    def forward_testic(self, image_input, text_inputs, key_padding_mask, max_index):
        len_i = image_input.shape[ 0 ]
        len_c = text_inputs.shape[ 0 ]
        IMG = torch.ones(len_i, 1, image_input.shape[ 2 ]).cuda()
        image_input = torch.cat((IMG, image_input), dim=1)  # B,N_I+[IMG],dim
        ...
        # #
        # image_atts = torch.ones(image_input.size()[ :-1 ], dtype=torch.long).cuda()
        # input_ids_0 = text_inputs
        # labels = input_ids_0
        # sample_index = np.random.choice(np.arange(0, 36))
        # print("sample_index:",sample_index)
        # image_input[ :, sample_index+1, : ] = torch.zeros(image_input.shape[ 2 ])
        # if max_index != None:
        #     select_index = torch.diagonal(max_index, dim1=0, dim2=1).t()  # len,36
        #     select_index = select_index[ :, sample_index ]  # len
        #     select_index = select_index + 1
        ...
        #     print(labels.shape)
        #
        # mlm_output = self.bert_model(input_ids_0,
        #                              attention_mask=key_padding_mask,
        #                              encoder_hidden_states=image_input,
        #                              encoder_attention_mask=image_atts,
        #                              return_dict=True,
