...
import numpy as np
from xbert import BertConfig, BertModel, BertForMaskedLM
...
class classifier_network5(nn.Module):
    ...
    def forward(self, image_input, text_inputs, idx, img_delta, txt_delta):
        ...
        score_cap = torch.zeros(len,len)
        for i in range(len):
            i_cap_out = cap_out[i].reshape(1,-1)
            score_cap[i] = F.cosine_similarity(i_cap_out,cap_out)
        score_cap = score_cap.cuda()
        ...
        cap_embeds_neg = []
        cap_neg_mask = []
        score_cap = torch.clamp(score_cap,0,1)
        for i in range(len):
            for j in range(len):
