...
from similiarity_network2 import classifier_network5
...
def main(opt,config):
    ...
    # train_features = np.load(opt.train_features_path,allow_pickle=True).item()
    ...
    for i_json in valid_caption_json:
        valid_captions.append(i_json[ 'caption' ])
        count = count + 1
        if count == 5:
            count = 0
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-5, weight_decay=1e-5)  # Adam优化器
    scheduler = CosineLRScheduler(optimizer, t_initial=5, lr_min=1e-6,
    ...
    for epoch_flag in range(config['epoch']):
        ...
        torch.save(model.state_dict(), "fin_model_flickr.pkl")

        if (epoch_flag % 2 == 0):
            score_val_i2t, score_val_t2i, ic_index, ci_index = evaluation(model, num_image, num_text, text_inputs,
                                                                          text_padding_mask, text_token_type_ids,
                                                                          image_inputs, device)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--train_features_path', default="/home/10501002/10501009/newflickr/train_img_flickr_dict.npy",
    #                     help='path to train_features')
    # parser.add_argument('--valid_features_path', default='/home/10501002/10501009/newflickr/valid_img_flickr_dict.npy',
    #                     help='path to valid_features')
    # parser.add_argument('--train_caption_path', default="/home/10501002/10501009/newflickr/train_caption_flickr.json",
    #                     help='path to train_caption')
    # parser.add_argument('--valid_caption_path', default="/home/10501002/10501009/newflickr/valid_caption_flickr.json")
    ...
    #                     help='Dimensionality of the word embedding.')
    parser.add_argument('--config', default='./configs/train_flickr.yaml')
