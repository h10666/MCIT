...
class classifier_network5(nn.Module):
    ...
    def forward(self, image_input, text_inputs, idx, img_delta, txt_delta):
        ...
        for i in range(len):
            i_sample_idx = i
            t_sample_idx = i
            #score_bf : 行：text   列：image
            while(int(pos_idx[i][i_sample_idx]) == 1):
                i_sample_idx = random.randint(0,len-1)
            while(int(pos_idx[t_sample_idx][i]) == 1):
                t_sample_idx = random.randint(0,len-1)
            img_inx.append(i_sample_idx)
            txt_inx.append(t_sample_idx)

        #get the negative sample
        text_neg_mask = [ ]
        for i in range(len):
            img_embeds_neg.append(image_input[ img_inx[ i ] ])
            text_embeds_neg.append(cap_input[ txt_inx[ i ] ])
            text_neg_mask.append(key_padding_mask[ txt_inx[ i ] ])

        text_neg_mask = torch.stack(text_neg_mask, dim=0)

        # negative text features combine
        # combine the negative sample
        # Text
