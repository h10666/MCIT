...
from net2 import classifier_network5
...
def train(epoch,model, data_loader, optimizer, scheduler,device, scaler_batch):
    ...
    for batch_idx, (image, caption, idx) in enumerate(data_loader):
        ...
        loss_num = model.forward(image, text_input_all,idx)
...
def main(opt):
    ...
    train_captions = [ ]
    train_image_ids = [ ]
    train_caption_json = json.load(open(opt.train_caption_path, 'r'))
    ...
    model = classifier_network5(text_encoder,opt.image_dim,opt.cap_dim,opt.hidden_dim)
    model = model.to(device)
    #msg = model.load_state_dict(torch.load("fin_model_net2.pkl"))
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4, weight_decay=1e-5)  # Adam优化器
    ...
    scheduler = CosineLRScheduler(optimizer, t_initial=5, lr_min=1e-6,
                                  decay_rate=1, warmup_lr_init=1e-4, warmup_t=1,
                                  t_in_epochs=True, )
    ...
    scaler_batch_num = 0
    for epoch_flag in range(opt.epoch):
        ...
        torch.save(model.state_dict(), "fin_model_net2.pkl")
