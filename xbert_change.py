        ...
        for i in range(start_layer, output_layer):
            ...
            if (i >= self.config.fusion_layer):
                ...
                if getattr(self.config, "gradient_checkpointing", False) and self.training:
                    ...
                    layer_outputs_ci = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(layer_module),
                        hidden_states,
                        attention_mask,
                        layer_head_mask,
                    ...
                    layer_outputs_ic = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(layer_module),
                        encoder_hidden_states,
                        encoder_attention_mask,
                        layer_head_mask,
                ...
                hidden_states = layer_outputs_ci[ 0 ]
                encoder_hidden_states = layer_outputs_ic[0]
                if use_cache:
                    next_decoder_cache += (layer_outputs_ci[ -1 ],)
                if output_attentions:
                    all_self_attentions = all_self_attentions + (layer_outputs_ci[ 1 ],)
