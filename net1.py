...
class classifier_network5(nn.Module):
    def __init__(self, text_encoder,image_dim, cap_emd, hidden_dim):
        ...
        transformer_encoder = nn.TransformerEncoderLayer(hidden_dim,4,image_dim,0.1)
        self.encoder = nn.TransformerEncoder(transformer_encoder,8)
    ...
    def forward(self, image_input, text_inputs, idx):
        ...
        co_inp = torch.cat([ text_embeds, image_embeds ], dim=1).transpose(0, 1)
        co_masks = torch.cat([key_padding_mask,image_atts],dim=1)
        co_out = self.encoder(co_inp,src_key_padding_mask=co_masks).transpose(0,1)
        co_cap,co_img = (co_out[:,:text_embeds.shape[1]],co_out[:,text_embeds.shape[1]:])
        ...
        co_neg_inp = torch.cat([text_neg_embeds,image_neg_embeds],dim=1).transpose(0,1)
        ...
        co_neg_cap,co_neg_img = (co_neg_out[:,:text_neg_embeds.shape[1]],co_neg_out[:,text_neg_embeds.shape[1]:])
        ...
        #co stream
        #postive
        img_txt_co_pos_out = self.bert_model(encoder_embeds=cap_input,
                                          attention_mask=key_padding_mask,
        ...
        #negative
        img_txt_co_neg_out = self.bert_model(encoder_embeds=text_embeds_neg_all,
                                          attention_mask=text_attention_neg_all,
